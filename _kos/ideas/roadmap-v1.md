...
forestage's primary use case: **session setup + statusline push**. Claude Code
owns the pane rendering. forestage does not read pane output. The API surface
forestage actually uses is narrow: connect, new_session, set_status_*, send_keys,
...
## Immediate (unblock forestage distribution)
...
- [ ] forestage Cargo.toml migrates from path → git tag or crates.io

→ Nodes: question-crates-publish, question-forestage-dep-source
...
- [ ] Confirm: does forestage need PaneExited handling? Or is it a one-shot launcher?
...
→ Nodes: question-forestage-pane-lifecycle, question-integration-tests
...
- [ ] Audit which unhandled notification types forestage actually needs
...
- [ ] Not needed for forestage's current use case; add when a consumer exists
...
- **capture-pane**: forestage doesn't need it (Claude Code owns the pane)
